...
-- 출신도(city)별 남자와 여자 인원수 출력 : emp 테이블
    -- city   성별   인원수
SELECT city, DECODE(MOD(SUBSTR(rrn,8,1),2),0,'남자', '여자') 성별, COUNT(*) FROM emp
GROUP BY city, DECODE(MOD(SUBSTR(rrn,8,1),2),0,'남자', '여자')
ORDER BY city;
...
-- 부서(dept)별 인원수와 부서의 월별로 생일인사람의 인원수 출력 : emp 테이블
...
-- 부서(dept)별 인원수 및 부서별 인원수가 전체 인원수의 몇 %인지 출력 : emp
    -- dept  인원수  백분율
SELECT dept, COUNT(*) 인원수, ROUND(COUNT(*)/(SELECT COUNT(*) FROM emp)*100) 백분율
FROM emp
GROUP BY dept;
...
SELECT dept, SUM(부장) 부장, SUM(과장) 과장, SUM(대리) 대리, SUM(사원) 사원  FROM (
    SELECT dept,
          COUNT(DECODE(pos, '부장', 1)) 부장,
          COUNT(DECODE(pos, '과장', 1)) 과장,
          COUNT(DECODE(pos, '대리', 1)) 대리,
